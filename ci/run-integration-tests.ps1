    ...
    try {
        ...
        if ($phpVersion -ge [version]'8.4') {
            Test-WordPress "Settings page loads (HTTP 200)" {
                Invoke-WebRequest -Uri "http://$addr/wp-login.php" -Method Post -SessionVariable session `
                    -Body @{log = "admin"; pwd = "admin"; "wp-submit" = "Log In"; redirect_to = "/wp-admin/"}
                (Invoke-WebRequest -Uri "http://$addr/wp-admin/options-general.php?page=51Degrees" -WebSession $session).StatusCode -eq 200
            }
        }

        Test-WordPress "REST endpoint responds (HTTP 200)" {
            (Invoke-WebRequest -Uri "http://$addr/wp-json/fiftyonedegrees/v4/json" -Method POST).StatusCode -eq 200
        }

        Test-WordPress "51Degrees JavaScript snippet is injected into page" {
            (Invoke-WebRequest -Uri "http://$addr/" -Headers @{
